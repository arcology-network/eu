...
func TestU256CumulativeParallelInitTest(t *testing.T) {
	...
	err, _, _ := DeployThenInvoke(targetPath, "multiprocess/multiprocess_test.sol", "0.8.19", "U256CumulativeParallelInitTest", "call()", []byte{}, false)
	if err != nil {
		t.Error(err)
	}
}
...
func TestRecursiveAssigner(t *testing.T) {
	...
	err, _, _ := DeployThenInvoke(targetPath, "multiprocess/multiprocess_test.sol", "0.8.19", "RecursiveAssignerTest", "call()", []byte{}, false)
	if err != nil {
		t.Error(err)
	}
}
...
func TestParaCumU256Sub(t *testing.T) {
	...
	err, _, _ := DeployThenInvoke(targetPath, "multiprocess/multiprocess_test.sol", "0.8.19", "ParaCumU256SubTest", "call()", []byte{}, false)
	if err != nil {
		t.Error(err)
	}
}
