...
type APIHandler struct {
	...
	writeCachePool *mempool.Mempool[*cache.StateCache]
	localCache     *cache.StateCache // The private cache for the current APIHandler
...
func NewAPIHandler(writeCachePool *mempool.Mempool[*cache.StateCache]) *APIHandler {
	api := &APIHandler{
...
func (this *APIHandler) New(writeCachePool any, localCache any, deployer ethcommon.Address, schedule any) intf.EthApiRouter {
	// localCache := writeCachePool.(*mempool.Mempool[*cache.StateCache]).New()
	api := NewAPIHandler(this.writeCachePool)
	api.SetDeployer(deployer)
	// api.writeCachePool = writeCachePool.(*mempool.Mempool[*cache.StateCache])
	api.writeCachePool = this.writeCachePool
	api.localCache = localCache.(*cache.StateCache)
...
func (this *APIHandler) Cascade() intf.EthApiRouter {
	...
	writeCache := cache.NewStateCache(this.localCache, 32, 1)

	// Use the current write cache as the read-only data store for the replicated APIHandler
	return api.SetStateCache(writeCache.SetReadOnlyBackend(this.localCache))
}

func (this *APIHandler) AuxDict() map[string]any { return this.auxDict }
func (this *APIHandler) StateCachePool() any     { return this.writeCachePool }
...
func (this *APIHandler) StateCache() any { return this.localCache }
func (this *APIHandler) SetStateCache(writeCache any) intf.EthApiRouter {
	this.localCache = writeCache.(*cache.StateCache)
	return this
}
...
func (this *APIHandler) ElementUID() []byte {
	pid := this.Pid()
	instanceID := slice.TrimTrail(pid[:8], 0) // Trim the trailing zeros to make it a valid UUID
...
func (this *APIHandler) Job() any {
	return this.eu.(interface{ Job() *workload.Job }).Job()
}
...
func (this *APIHandler) PrepayGas(initGas *uint64, gasRemaining *uint64) (uint64, bool) {
	job := this.eu.(interface{ Job() *workload.Job }).Job()
	...
	callee, selector := *job.StdMsg.Native.To, codec.Bytes4{}.FromBytes(job.StdMsg.Native.Data)
	prepaidGasAmount := this.GetRequiredAmount(callee, selector)
	...
	path := (&stgcommon.PathBuilder{callee, selector, stgcommon.ETH_PATH}).ProfileField(stgcommon.PREPAYERS)
	targetPath := path + hexutil.Encode(job.StdMsg.Native.From[:]) + ":" + strconv.FormatUint(txID, 10)
...
func (this *APIHandler) UsePrepaidGas(gasRemaining *uint64) bool {
	...
	callee, selector := *job.StdMsg.Native.To, codec.Bytes4{}.FromBytes(job.StdMsg.Native.Data)
	path := (&stgcommon.PathBuilder{callee, selector, stgcommon.ETH_PATH}).ProfileField(stgcommon.PREPAYERS)
	payers, _, _ := cache.Read(txID, path, commutative.Path{})
...
func (this *APIHandler) RefundPrepaidGas(gasLeft *uint64) bool {
	...
	if !job.StdMsg.IsDeferred && job.Err != nil {
		...
		// Get the path where all the prepayers are stored.
		payerDir := (&stgcommon.PathBuilder{callee, selector, stgcommon.ETH_PATH}).ProfileField(stgcommon.PREPAYERS)
		key := payerDir + hexutil.Encode(job.StdMsg.Native.From[:]) + ":" + strconv.FormatUint(txID, 10)
		if _, err := cache.Write(txID, key, nil); err != nil {
			return false
		}

		payer := hexutil.Encode(job.StdMsg.Native.From[:]) + ":" + strconv.FormatUint(txID, 10)
		payerPath, _ := stgcommon.GetParentPath(payerDir + hexutil.Encode(job.StdMsg.Native.From[:]))
	...
	if !job.StdMsg.IsDeferred {
		...
		if job.Err != nil { // Parallel Execution failed, refund the prepaid gas directly.
			*gasLeft += job.PrepaidGas
			...
			payerDir := (&stgcommon.PathBuilder{callee, selector, stgcommon.ETH_PATH}).ProfileField(stgcommon.PREPAYERS)
			targetPath := payerDir + hexutil.Encode(job.StdMsg.Native.From[:]) + ":" + strconv.FormatUint(txID, 10)
	...
	for _, payer := range payers {
		...
		if _, err := cache.Write(txID, payerDir+payer, nil); err != nil {
			return false
		}
	}
	return false
...
func (this *APIHandler) GetTxContext() (uint64, *cache.StateCache) {
	return this.GetEU().(interface{ ID() uint64 }).ID(),
		this.StateCache().(*cache.StateCache)
}

// Get the required prepayment amount.
func (this *APIHandler) GetRequiredAmount(addr [20]byte, selector [4]byte) any {
	txID, cache := this.GetTxContext()
	path := (&stgcommon.PathBuilder{addr, selector, stgcommon.ETH_PATH}).ProfileField(stgcommon.DEFERRED_PAYMENT)
	prepaidGasAmount, _, _ := cache.Read(txID, path, new(commutative.Uint64))
