...
func (this *TxAccessRecords) Size() uint64 {
	return this.HeaderSize() +
		...
		codec.UINT64_LEN +
		statecell.StateCells(this.Accesses).Size()
}
...
func (this *TxAccessRecords) EncodeTo(buffer []byte) int {
	...
	offset += codec.Bytes(statecell.StateCells(this.Accesses).Encode()).EncodeTo(buffer[offset:])
	return offset
}
...
func (this *TxAccessRecords) Decode(buffer []byte) *TxAccessRecords {
	...
	uns := statecell.StateCells(this.Accesses).Decode(fields[2])
	this.Accesses = uns.(statecell.StateCells)
	return this
}
